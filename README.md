...
辨识分两步。进入每个单词的第一页时会自动朗读，且只显示原始单词拼写；`Q` 选择认识、`E` 选择不认识，两种选择都会显示答案、以分音节词形展示单词（如 `ac·co·lade`），并再次朗读当前词。答案页按 `N` 进入下一词；若第一页选了“认识”，还可按 `M` 改判为不认识并立即前进。下一词出现时开始新一轮自动朗读。`←` 回看上一词，`Delete` 或 `Backspace` 将当前词从词表移出；回看与重复提交不会重复计分。
