...
辨识分两步。进入每个单词的第一页时会自动朗读；`Enter` 选择认识、`N` 选择不认识，两种选择都会显示答案并再次朗读当前词。第一页选“不认识”后，答案页只提供 `Enter` 进入下一词；第一页选“认识”后，答案页还可按 `N` 改判为不认识并立即前进。下一词出现时开始新一轮自动朗读。`←` 回看上一词，`Delete` 或 `Backspace` 将当前词从词表移出；回看与重复提交不会重复计分。
