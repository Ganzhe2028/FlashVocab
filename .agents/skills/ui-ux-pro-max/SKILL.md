...
This repository vendors the skill at `.agents/skills/ui-ux-pro-max`. Run the commands below from the repository root so the skill never depends on a user-global install:

```bash
python3 .agents/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain <domain>
```

If `python3` is not found, try `python`, then `py -3`. Requires Python 3.x and no external dependencies.
...
# Step 4: stack guidelines
python3 .agents/skills/ui-ux-pro-max/scripts/search.py "suspense streaming bundle" --stack react
```
