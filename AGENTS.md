...
- A fresh visitor gets an Import-first preparation page, an optional one-minute explanation, and contextual prompts during the first recognition and spelling actions. The persistent help section can restart those prompts; there is no blocking slide tour or restored 2.x Guidebook page.
- The quiet header contains brand, More, the GitHub mark, and—outside the preparation page—Import, plus Undo when available. Import opens the deck section directly; More opens help, while help/settings/deck remain separate sections in one right-side panel. The panel traps Tab focus and keeps its section navigation visible while scrolling. The header fades on learning keys or 2.5 seconds of pointer inactivity, returns on pointer/Tab, and remains active while the overlay is open. Reduced motion, narrow layouts, and 200% zoom must remain usable.
